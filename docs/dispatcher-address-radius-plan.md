...
1. **`volunteer_coords.json` (server-side, gitignored, NEW):** per qualifying
   ...
   address_hash, geocoded_at, first_name}`. **First name only (single token); no
   last name, no address** — last names never leave the pipeline. Whether the
   first name reaches a Worker response is gated by `SHOW_VOLUNTEER_FIRST_NAME`
   (default ON). This is the only place coordinates live.
